...
describe('visual baseline pruning inventory', () => {
  it('shares the exact 44-case executable matrix', () => {
    expect(VISUAL_CASE_MATRIX).toHaveLength(44);
    expect(VISUAL_BASELINE_PATHS).toHaveLength(44);
    expect(new Set(VISUAL_BASELINE_PATHS).size).toBe(44);
  ...
  it('reports retained, missing, and removable files without pruning', async () => {
    ...
    expect(inventory.retained.size).toBe(88);
