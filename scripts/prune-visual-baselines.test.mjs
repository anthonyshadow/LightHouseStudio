...
describe('visual baseline pruning inventory', () => {
  it('shares the exact 38-case executable matrix', () => {
    expect(VISUAL_CASE_MATRIX).toHaveLength(38);
    expect(VISUAL_BASELINE_PATHS).toHaveLength(38);
    expect(new Set(VISUAL_BASELINE_PATHS).size).toBe(38);
  ...
  it('reports retained, missing, and removable files without pruning', async () => {
    ...
    expect(inventory.retained.size).toBe(76);
