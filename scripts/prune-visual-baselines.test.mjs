...
describe('visual baseline pruning inventory', () => {
  it('shares the exact 31-case executable matrix', () => {
    expect(VISUAL_CASE_MATRIX).toHaveLength(31);
    expect(VISUAL_BASELINE_PATHS).toHaveLength(31);
    expect(new Set(VISUAL_BASELINE_PATHS).size).toBe(31);
  ...
  it('reports retained, missing, and removable files without pruning', async () => {
    ...
    expect(inventory.retained.size).toBe(62);
