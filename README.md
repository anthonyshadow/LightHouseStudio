...
- Optional, automatically optimized OpenAI `gpt-image-2` or BFL `flux-2-pro` previews, durable local uploads, source-image composition, and instructed editing
...
| `REFERENCE_IMAGE_PROVIDER`           | No                                     | Server-only image-provider selector, `openai` or `bfl`; defaults to `openai`. Selection occurs once at startup and never silently falls back.                                                                                                                                                                                            |
