...
const ProjectListSection = ({
  ...
  const theme = useTheme();
  const query = useProjectList(lifecycle);
  ...
  return (
    <section css={listSectionStyles(theme)} aria-labelledby={`${lifecycle}-projects-heading`}>
      <header>
        <h3 id={`${lifecycle}-projects-heading`}>{archived ? 'Archived' : 'Active Projects'}</h3>
      ...
      {projects.length > 0 ? (
        <ul
          ...
          aria-label={`${archived ? 'Archived' : 'Active'} Projects`}
        >
...
const ProjectsWorkspace = () => {
  ...
  const quickStart = async () => {
    setCreateError(null);
    try {
      const current = await controller.createMutation.mutateAsync();
