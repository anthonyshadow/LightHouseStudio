...
const renderPreview = (onError = vi.fn()) => {
  const Harness = () => {
    const controller = useVoicePreviewController(onError);
    return (
      <StudioDesignProvider>
        <button type="button" onClick={() => void controller.load(item)}>
          Preview Studio Star
        </button>
        <VoicePreview
          ...
          reportPlaybackError={controller.reportPlaybackError}
        />
      </StudioDesignProvider>
    );
  };
...
describe('VoicePreview', () => {
  it('contacts the provider only after an explicit action and revokes the one owned URL', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Preview Studio Star' }));
  ...
  it('aborts a pending preview when its workspace owner unmounts', async () => {
    const user = userEvent.setup();
  ...
  it('reports playback failures without exposing provider details', async () => {
    const user = userEvent.setup();
    const onError = vi.fn();
    fetchVoicePreview.mockResolvedValue(new Blob(['preview'], { type: 'audio/mpeg' }));
    ...
    await user.click(screen.getByRole('button', { name: 'Preview Studio Star' }));
    fireEvent.error(await screen.findByLabelText('Listen to Studio Star preview'));

    expect(onError).toHaveBeenCalledWith(item);
  });
});
