...
describe('CharacterPromptWorkshop', () => {
  ...
  it('round-trips ethnicity, skin tone, body shape, and hair color independently through Advanced', async () => {
    const user = userEvent.setup();
    const { onUse } = renderWorkshop({
    ...
    await user.type(screen.getByLabelText('Ethnicity'), 'South Asian');
    ...
    expect(action?.prompt).toContain('Ethnicity: South Asian.');
