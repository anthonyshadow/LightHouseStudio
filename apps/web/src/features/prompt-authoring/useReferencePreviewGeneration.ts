...
export const useReferencePreviewGeneration = (callbacks: ReferencePreviewGenerationCallbacks) => {
  ...
  const generate = useCallback(async (input: ReferencePreviewGenerationInput): Promise<void> => {
    ...
    try {
      let optimization =
        optimizationRef.current?.sourceKey === sourceKey ? optimizationRef.current.response : null;
      if (!optimization) {
