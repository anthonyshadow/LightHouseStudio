...
export interface StudioExitGuardProps {
  ...
  /** A presented artifact exists at all — what decides whether a Project has an unaccepted draft. */
  readonly hasTemporaryTake: boolean;
  /**
   * The presented artifact exists only in this browser.
   *
   * Narrower than `hasTemporaryTake` on purpose. A Saved Video opened in the Studio is presented
   * too, and prompting to "discard temporary work" on the way out of it would be offering to throw
   * away a file that is already on the server — so what counts is whether leaving would actually
   * lose something. The Project branch has always drawn this distinction through acceptance; this
  ...
  /**
   * A provider result is in hand and has not been saved anywhere.
   *
   * Separate from `hasTemporaryTake`, which covers any presented artifact and deliberately does not
   * arm the unload prompt — a recording can be made again. This one cannot: the server hands the
   * generated bytes over once and does not keep them, so closing the tab throws away work that was
   * paid for and would have to be paid for again.
   */
  readonly unsavedProviderResult?: boolean;
 ...
 *
 * Every destination that mounts the Studio runtime needs a key here, because a key of `null` means
 * "nothing to leave" and the guard stands aside. `/studio/<uuid>` holds exactly the same in-memory
 * work as the create route — a dirty local edit, a voice treatment, an unsaved take — so it is keyed
 * per Saved Video: moving between two of them abandons the first one's edits just as surely as
 * leaving for the Dashboard does.
 */
const studioWorkspaceKeyFromPath = (pathname: string): string | null => {
  ...
  const savedVideoId = studioVideoIdFromPath(pathname);
  if (savedVideoId !== null) return `saved-video:${savedVideoId}`;
