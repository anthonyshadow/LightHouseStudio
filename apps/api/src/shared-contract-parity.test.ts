...
import {
  ...
  PROJECT_SOURCE_LIMIT as CONTRACT_PROJECT_SOURCE_LIMIT,
...
describe('snapshot v3 parity', () => {
  it('keeps the snapshot versions, the clip limit and the trim minimum in parity', () => {
    ...
    expect(CONTRACT_PROJECT_SOURCE_LIMIT).toBe(PROJECT_SOURCE_LIMIT);
...
it('mirrors the audio level ceiling between the contract and the domain', () => {
  expect(CONTRACT_VIDEO_EDIT_AUDIO_LEVEL_MAX).toBe(VIDEO_EDIT_AUDIO_LEVEL_MAX);
});
