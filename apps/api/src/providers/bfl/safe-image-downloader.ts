...
export const isPublicImageAddress = (address: string): boolean => {
  ...
  if (mappedIpv4 !== undefined) return isPublicImageAddress(mappedIpv4);
...
const defaultResolver: ImageHostnameResolver = async (hostname) => {
...
export class SafeBflImageDownloader {
  ...
  async #download(
    ...
    if (selected === undefined || addresses.some((entry) => !isPublicImageAddress(entry.address))) {
      throw providerError('invalid-response');
    }
    ...
    return new Promise<DownloadedProviderImage>((resolve, reject) => {
