...
| `upload-record-to-source`      | Explicit panel action closes the panel; the persistent stage previews, records, warns/stops at 270/300 seconds, finalizes, reviews, and adopts the normalized local artifact without provider traffic; no inline player participates in capture |
