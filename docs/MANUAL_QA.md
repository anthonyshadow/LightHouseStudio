...
| `creative-library-recovery` | With Neon authoritative, edit the creative library in a second browser profile so both copies diverge; confirm the paused-sync notice appears on every protected route, that **Save current copy** and **Reload account copy** both confirm before overwriting, that each resolution resumes syncing, and that a transport failure offers only **Try again**                                                                                                                                                  |
| `studio-library-routing`    | `/dashboard`, `/campaigns`, `/projects`, and direct Videos, Characters, Outfits, and Voices libraries keep one mounted authenticated shell; `/assets` replaces to the last-used/default Videos library; legacy organization URLs and the singular `/campaign` normalize; no media stage is mounted on organization pages; Back/Forward, Escape, bottom navigation, and return focus behave correctly. Tab switches replace the route, and closing returns to the opening origin without another history entry |
...
| `project-source-entry`      | An empty Project overview shows the workflow steps and the Source task; the primary action reads Add source; Record, Upload and Use Saved Video each end in the workspace with the original accepted; a source-bearing overview shows neither the Source task nor Add source and issues no source-content request                                                                                                                                                                                             |
| `project-source-vs-assets`  | Videos ⋯ ▸ Use as Project source sets an empty Project's original and refuses one that already has a source; attached Assets state that they are not the source; adopting an attached Video reads Use as Project source (confirmed) or Use as working media by Project state                                                                                                                                                                                                                                  |
| `record-intent-entry`       | Quick Create ▸ Create Asset ▸ Video ▸ Record Video starts capture; navigating away and repeating the same entry starts capture again rather than landing on an idle Studio                                                                                                                                                                                                                                                                                                                                    |
| `workspace-task-progress`   | The workspace masthead marks the current step; a Project opens on the step it is up to; a phase change mid-session does not move the open panel; choosing a task writes `?task=` and reopens there, survives login, and never triggers the exit guard                                                                                                                                                                                                                                                         |
| `video-gallery-states`      | Empty/loading/error/missing/placeholder/populated/load-more states avoid eager video requests; thumbnail Preview darkens the gallery, traps/returns focus, detaches on close, and remains usable at all five viewports and 200% text. On tablet/phone the title-search × clears in place, Filters aligns with the input, the filter sheet does not clip Sort, and Clear/Show remain on one row                                                                                                                |
| `saved-character-outfit`    | Character and Outfit Assets hydrate the same owner-scoped records in a fresh signed-in profile; Use follows the existing handoff; delete removes relationships without provider work or unintended immutable-byte deletion                                                                                                                                                                                                                                                                                    |
...
| `upload-local-save`            | A zero-step H.264 MP4/MOV or VP8 WebM source previews and saves with no external request; download is available afterward from Videos in Assets                                                                                                                            |
...
| `upload-record-to-source`      | Control-bar and panel Record intents use the Studio stage, warn/stop at 270/300 seconds, finalize, and adopt the normalized local artifact into the editor without provider traffic; no inline player participates in capture                                              |
...
- Apply 16:9 and 9:16 at every physical target. Verify the same stage changes shape,
...
- A conversion failure or dropped primary video/audio track exposes no raw recorder URL or
  saveable fallback.
- Playback stays on the Studio stage; Latest Take opens only through **Take** and contains no
