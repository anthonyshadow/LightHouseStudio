...
| ~~31~~ | Studio stage sizing and the rail footnote      | LF-S03, LF-S05         | The privacy line moved to the session column and the help explainer, from one constant; narrower side columns grew the 1440px frame from 661×372 to 718×404. |
...
| ~~34~~ | Copy tone                                      | LF-W01, LF-W02, LF-P05 | One positive statement instead of three negations, one owned sentence instead of three five-item lists, and History as a record rather than step 4.          |
...
**A cleanup pass followed the twelve items** — reuse, simplification, efficiency and altitude,
reviewed across the whole Tier 3–4 range. It found one real defect: item 30 decremented the
character-builder footer's track count for the button it removed but not for the `Back` button it
made conditional, so step two wrapped Save onto its own row. It also gave one owner to the two
sentences a session owes the operator, replaced six hand-written loading scaffolds and two
collapsed-archive blocks with `LoadingPlaceholder` and `CollapsedListSection`, returned the
`ActionMenu` trigger's size to its own primitive, fixed a missing `display: grid` in `VideoPlayer`
that left a baseline gap under the video on all five surfaces, and swept the leftovers each item
had left behind. The one finding deliberately skipped is measured rather than argued: the entry
page's `AppIcon` import costs 2.0 kB gzipped on the eager chunk, which is the right price for one
icon system.
