...
| Creative resource | A reusable input or direction such as a Character, Character variant, Outfit, Voice, or prompt       | Characters, variants, Outfits, and Voices are implemented with different persistence lifecycles; Recipe-shaped data is compatibility-only |
| Library           | An organized collection intended for finding and reusing retained records                            | Assets groups Videos, Characters, Outfits, and Voices                                                                                     |
