...
| Character AI              | Start camera → Start AI → Start with selected character                     | Character is primary; transfer/usage/maximum/Stop disclosure and live cap state are present |
...
| Virtual Try-On            | Start AI/Dock → VTO → prompt/image/both → Start                             | Secondary/beta with one-garment guidance and no fit/size claim                              |
| Structured prompt         | Workshop → Add/Replace/Restyle → use or save                                | Clear advanced task                                                                         |
| Record                    | Live stage → Record → Stop → Finalizing                                     | Dominant Stop and 300-second policy are implemented; physical evidence remains              |
| Review                    | Playback replaces live stage → Download/Voice/Release/Discard               | Correct media continuity and data-loss protection                                           |
| Local voice               | Review → Voice → Warm/Clear/Robot → process/use                             | Clear and provider-free                                                                     |
| ElevenLabs voice          | Voice → explicit contacts-provider action → browse/preview/apply            | Strong disclosure, breadcrumb, and progressive technical detail                             |
...
| Character selector    | Selected/none state, create/edit/choose                                                   | Right drawer; mobile full-screen                            | Saved-character intent reaches Characters; physical evidence pending |
| AI chooser            | Character primary; VTO secondary/beta; contextual capability recovery                     | Full-screen while local media remains active                | Moderated comprehension and live qualification remain                |
...
| Prompt Workshop       | Constrained Add/Replace/Restyle authoring; labeled advanced in the rail                   | Wide drawer → tablet bottom → mobile full-screen            | Terminology still needs moderated observation                        |
...
| Character Builder     | Upload, choices, one preview, anchored narrow Review/Generate, save                       | Full-screen; desktop sticky preview                         | Physical safe-area/keyboard/assistive evidence remains               |
...
| Voice Treatments      | Local choices, breadcrumb, progress, errors, progressive compatibility detail             | Bottom sheet over playback                                  | Physical/screen-reader comprehension evidence remains                |
| Voice Browser         | Explicit provider contact, breadcrumb, usage context, search/error/paging                 | Nested right drawer                                         | Deep stack remains deliberate and needs physical validation          |
...
- Builder collapses to one column and anchors a focus/scroll shortcut to the one preview/generation
  region after the final configuration section.
...
- Validating whether the action-first rail descriptions reduce Dock, Shelf, Workshop, recipe,
  working-draft, and applied-recipe terminology cost.
...
- Confirming the anchored mobile Builder shortcut on physical safe-area/software-keyboard states.
...
- Validating the Take Review → Voice Treatments → Saved voices breadcrumb with screen readers.
...
- No runtime state from the Wave 6 usability pass is missing; physical-device, assistive-tech, and
  moderated comprehension evidence remains.
...
| UX-003 | **Runtime implemented:** narrow Builder exposes one-DOM Review & Generate focus/scroll shortcut              | Generation and result feedback are reachable without duplicated state                  | Builder form/styles plus component and 320/390/834/200%-text browser coverage             | Complete named physical, safe-area, software-keyboard, and assistive-technology evidence                                                                       | Runtime/automated complete                  | M                   | Medium            | Physical evidence remains                                      |
| UX-004 | **Runtime implemented:** dismissible first-take sequence stays inside the idle stage                         | New users can see the local preview → Character → Record → Voice → Download path       | Session-only Studio state; no route, persistence, or analytics                            | Validate comprehension through moderated observation before terminology/onboarding expansion                                                                   | Runtime/automated complete                  | M                   | Low–Medium        | Moderated evidence remains                                     |
| UX-005 | **Runtime implemented:** established tool nouns now have action-first descriptions                           | Users get plain setup/review/change/reuse verbs without an IA rename                   | Creative rail accessible descriptions and responsive collapse                             | Test terminology before any wholesale rename                                                                                                                   | Runtime/automated complete                  | S–M                 | Low               | Moderated evidence remains                                     |
| UX-006 | Recording now collapses to Stop; broader long-label/reflow and physical evidence is pending                  | Unqualified text/device combinations could still compromise reachability               | Control-bar rendering; 200%-text mobile E2E; reviewed Wave 6 Darwin baselines             | Finish Linux baseline review and physical long-label, safe-area, and 200% checks                                                                               | P1                                          | S                   | Low               | Before mobile/unassisted MVP                                   |
| UX-007 | **Implemented:** header truth and chooser contextual availability share capability state                     | Users see why provider Start is absent while local preparation remains available       | Header/chooser derivation, rendering, and recovery-copy tests                             | Preserve configured/limited/available-to-try language and never imply provider health                                                                          | Complete runtime/automated                  | S                   | Low               | Live qualification remains                                     |
| UX-008 | **Runtime implemented:** Voice keeps breadcrumbs and concise transfer/usage copy; secondary details collapse | Users retain Take → Voice → saved-voice context while technical detail stays available | Voice Treatments/Browser/Library component and dynamic axe coverage                       | Complete screen-reader and physical mobile comprehension checks                                                                                                | Runtime/automated complete                  | M                   | Medium            | Manual evidence remains                                        |
| UX-009 | **Implemented:** post-download actions now say Release / Close and release                                   | Users can distinguish temporary-artifact cleanup from ordinary overlay closure         | Take Review actions, cleanup tests, and current journey copy                              | Preserve the download gate, truthful dispatch language, object-URL cleanup, and confirmed Discard                                                              | Complete runtime/automated                  | XS–S                | Low               | Complete                                                       |
...
| UX-013 | **Pilot disclosure implemented:** detach/delete UI does not erase immutable local reference bytes            | Upload, Save, Reset, detach, and character-record deletion now state retained bytes    | Builder/Shelf copy tests; operator checklist and disposable exact-leaf retirement drill   | Preserve truthful detach language and per-participant verified cleanup. Later add coherent erase-all/relationship-aware deletion, not unsafe orphan deletion   | Pilot runtime/automated complete; public P1 | S–M pilot; L public | High for deletion | Real participant checklist required; full manager deferred     |
...
UX-009 is complete. VTO and Voice are included, so their remaining comprehension, trust, and
responsive polish are P1 release work rather than conditional.
...
five Darwin and Linux baseline viewports. Wave 6 adds and reviews the ten intentional Darwin
first-use/Character/VTO/Builder/Voice changes; equivalent Linux review remains. UX-003 runtime work
is complete, but the physical/reflow evidence portion of UX-006 is not fixed merely by accepting
screenshots.
