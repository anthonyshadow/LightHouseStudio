...
| Reuse            | Character selector, Recipe Shelf: Saved/Recent/Characters                 | Select, search, edit, rename, delete, and reuse browser-local work              | Strong model; saved-character entry does not honor intent |
...
Use plain verbs in first-success guidance: Start camera, choose a character, set up Virtual Try-On, browse saved characters. `Systems ready` should change to truthful capability language such as `Integrations configured`; configuration is not health, reachability, quota, or entitlement.
...
| Select saved character    | Header/AI chooser → Choose saved character → Shelf → Characters → Use       | **Defect:** entry does not select Characters in a clean session                 |
| Character AI              | Start camera → Start AI → Start with selected character                     | **Trust gap:** direct Start lacks the Dock disclosure; session cap is invisible |
...
- “Choose saved character” opens a generic Shelf state rather than the Characters category.
- Header “ready” language describes configured capability, while action failures still depend on entitlement, quota, and reachability.
- Recipe Dock contains provider disclosure, but direct Start from the chooser does not.
...
- Direct-start provider/usage disclosure.
- AI-session maximum, elapsed/remaining state, near-expiry warning, and expected-limit completion.
