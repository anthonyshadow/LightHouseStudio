...
| ---------------------------------------------------- | --------------------------------------------------------------------------------------- | --------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `01-studio/initial-closed.png`                       | Initial Studio, no dialog open, camera off, no character selected, first-take cue shown | All five              | `StudioHeader`, `MediaStage`, dismissible first-take cue, idle controls, action-first rail | Protects the real first impression, primary camera action, and local → Character → take path  | Fixed capabilities/time; seeded saved character remains unselected and visually background-only | The cue is session-only; a screenshot cannot prove dismissal or first-success comprehension                                     |
...
| `01-studio/recording-active.png`                     | Local preview recording; dominant Stop Recording is the sole stage action               | All five              | Stage recording indicator/timer, control bar, capture status                               | Protects the highest-consequence live control state and narrow density                        | Synthetic media and `MediaRecorder`                                                             | Does not prove timeout behavior, physical touch/safe areas, long-duration memory, final data, or codec behavior                 |
| `01-studio/ai-experience-choice.png`                 | Local preview active; Start AI chooser open                                             | Desktop, small mobile | `AIExperienceChooser`, Character-primary card, VTO secondary/beta card, stage beneath      | Protects the primary/secondary AI hierarchy and full-screen mobile presentation               | Synthetic local media; configured capabilities                                                  | The configured state cannot prove recovery copy for loading, unavailable, or failed capability checks                           |
...
| `02-character-builder/combined-reference-ready.png`  | Builder with uploaded source and successful generated combined preview                  | Desktop, small mobile | Builder inputs, one generated preview, narrow Review/Generate shortcut, Save action        | Protects the most layout-intensive Character state and the single-DOM narrow review path      | Stable 1×1 PNG upload; mocked optimizer/reference-generation response                           | Prompt-only, image-only save, loading, stale, regeneration, and error variants remain behavioral/component coverage             |
| `03-character-library/saved-character-selection.png` | Enter saved-character Shelf and show Characters category with deterministic character   | Desktop, small mobile | Character selector entry, `RecipeShelf`, category control, character card/list             | Protects the collection users expect from “Choose saved character”                            | Seeded `CreativeAssetStore`                                                                     | Fixture requires Characters to be selected immediately; the complete browser journey covers both entry points through Use/Start |
...
| `06-voice/voice-browser-loaded.png`                  | Recorded take → Voice Treatments → Voice Browser loaded with deterministic saved voice  | Desktop               | Compact treatment choice, breadcrumb, progressive disclosure, Voice Browser/list           | Protects the deepest provider-adjacent overlay while preserving provider/usage context        | Synthetic take; mocked ElevenLabs availability/list with `Northstar Narrator`                   | No live preview/conversion/remux, search/pagination matrix, provider billing, or zero-retention proof                           |
...
The new initial, selected-character, and settled-review scenario definitions address the first
three test-state problems through explicit setup/readiness assertions. Wave 1 collapses recording
to a sole dominant Stop action; all five affected Darwin and Linux baselines were regenerated and
visually reviewed on 2026-07-28. Physical-device and assistive-technology proof remains separate.

### Current unresolved product/test boundaries

- `openSavedCharacters` requires the Characters tab to be selected immediately after **Choose
  saved character**. It does not repair the state in the fixture; the end-to-end Character journey
  also covers both entry points through **Use → Start**.
- Static screenshots cannot prove the implemented timeout, touch/pointer/focus recovery, or
  never-hidden behavior; component and functional E2E own those assertions.
- The ten intentional Wave 6 Darwin baseline changes were reviewed on 2026-07-28. Equivalent Linux
  baseline regeneration/review remains required before the visual release gate can be called
  complete.
