...
are Campaigns, empty Projects, Saved Videos and their immutable versions, reusable Characters and
...
version 2 strictly migrates v1 Projects to null membership and adds Campaigns/receipts without a
default row. It is atomically replaced with a validated backup. Create and membership mutations publish a
...
The shared owner lock/journal makes Campaign create and Project membership atomic without a second
...
active/archived Campaign-membership list indexes used by grouped Project views. See
...
| Local persistence           | Campaign/Project metadata and create journals, saved-video aggregates/versions, thumbnails, references, saved voices, and safe processing traces                            | Default `local` authority under `LIGHTFRAME_DATA_DIR`; Campaign/Project and saved-video metadata use versioned owner namespaces, atomic publication, validated recovery, durable idempotent receipts, and owner checks. `shadow` retains local Campaign/Project authority. Sessions are process-memory.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Projects                    | `GET/POST /api/projects`, `GET/PATCH /api/projects/:projectId`, `POST` archive/restore subroutes, and `POST /api/projects/:projectId/campaign`; create requires `Idempotency-Key`, mutations use Project-version CAS, and lists support bounded lifecycle plus Campaign/No Campaign filtering                                                                          |
