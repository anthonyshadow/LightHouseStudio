...
a left rail at `48rem` and above, and a compact top bar plus a four-item bottom navigation below
it. The rail carries Dashboard, Studio, Projects, Campaigns and Assets; the bottom bar carries the
four surfaces an operator stands on, because the asset libraries are overlays opened over the
current surface rather than places to be, and the Dashboard carries the way into them.
...
receipts. `lastSuccessfulOutput` must match an existing output relation retained by the same
...
The broker assigns one immutable deadline when it creates a job: `createdAt + 60 minutes`, computed
before the provider is contacted rather than when the provider accepts. That same deadline covers
validating, submitting, queued, processing, retrieving, and ready states; status reads, retries, and
the transition to ready never extend it. A service-owned nearest-deadline timer enforces expiry
without requiring another request, preserves an expired in-memory tombstone, and guards late
provider work from restoring state or bytes. Explicit release, the acknowledged abandon action, or
broker shutdown may delete local state earlier, and so may successful delivery of a Project-linked
result, which is already in the owner byte store by then; a delivered standalone result is retained
until its deadline instead, so an interrupted download can be repeated without a second paid
submission. A content stream admitted before the deadline
...
A bounded in-process progression tick, owned by the same process that owns the jobs, moves accepted
jobs without a client: each pass polls at most the configured per-provider active ceiling, Project
work first, retains the Project results that became ready into the owner byte store, and — at most
once a minute — asks the AI usage reconciler to close ledger rows whose jobs settled unobserved.
`VIDEO_JOB_PROGRESSION_INTERVAL_MS=0` turns it off, and jobs then move only while a client watches.
The tick submits nothing, retries no paid call, and promotes nothing into a Project's current cut:
adopting a result as the current cut stays an operator action on their next visit. It assumes a
single process per data directory (D9); it takes no lock.
...
| AI usage ledger             | Authenticated `GET /api/account/ai-usage` returns one owner's video-transformation submissions for a requested window: outcome counts for the whole window and a cursor-bounded page of rows carrying operation, provider, outcome, instants and duration. Owner comes from the session subject, the window is capped, the cursor is bound to it, and the response is never cached                                                                                                                                                       |
...
signup, analytics, public tenancy, or billing. The only unattended work is the bounded video-job
progression tick above, which moves jobs this process already owns; there is still no general
durable worker queue, no queue table, and no second process. Local mode uses files
