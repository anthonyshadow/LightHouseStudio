...
`StudioApp.tsx` is the composition and URL-dispatch boundary. Its dependency-free resolver sends `/` to Guided (resuming the latest safe checkpoint), `/advanced` to the original studio, and `/projects` to the Guided project browser. `?new=1` starts a fresh Guided project. `VITE_CHARACTER_FLOW_ROLLOUT=all` is the default; `opt-in` requires `/guided` or `?characterFlow=guided`, while `off` sends every non-Advanced entry to Advanced. There is no global client store or third-party router.

`GuidedExperience` owns the all-user Create → Live → Record → Voice → Download journey. A pure typed reducer accepts explicit events, emits side-effect descriptions, and rejects async results whose operation id or base project revision is stale. It restores only stable checkpoints. The Create stage uses a versioned visual catalog with nine starters, four presentation profiles, shared skin-tone choices, six profile-aware suggestions where applicable, and lossless custom values. Changing gender recomputes suggestions without clearing an existing selection.

`StudioExperience` remains the Advanced composition boundary and owns the session, recording, voice-processing, recipe-repository, active-overlay, and tool-draft controllers. Capability hooks own long-lived resources, while visual leaves receive typed controllers and callbacks.
...
5. The broker validates Host, an Origin with the exact same loopback host and port, the body, the session profile, and the model allowlist. It then asks Decart for an origin/model-scoped credential with a five-minute connection-start TTL. Advanced credentials allow a five-minute active session; Guided credentials allow seven minutes so a healthy session can cover its countdown and five-minute maximum take.
...
Guided maps its visual selections into the same canonical Character draft, including first-class Skin Tone, Body Shape, Hairstyle, and Hair Color fields; Advanced exposes those text controls without removing its existing options or Add, Replace, and Restyle intents. Save Character validates first and then presents an unselected choice: Continue with Prompt Only, Generate Reference & Continue, or Keep Existing Reference when a compatible non-stale asset exists. Prompt-only activation does not call image generation. Generation begins only from its final explicit action, and a failure preserves the draft and permits deliberate prompt-only continuation.
...
Advanced Studio owns one temporary take at a time; it does not implement take history, rename, trim, or persistent media save. `Download take` dispatches a browser download but leaves playback active. Because completion is not browser-observable, successful synchronous dispatch only enables `Close take`; Close revokes original and processed URLs and returns to private idle. A failed dispatch leaves review intact. Confirmed Discard performs the same cleanup without download. Until Close or Discard, the persistent main-stage player is the only take player.

The default Guided flow reuses the same session, recording, and voice-processing owners but checkpoints serializable state through a typed reducer and versioned IndexedDB project repository. Each record includes the complete canonical character draft and optional guided provenance; the finalized original, optional audio sidecar, and processed media are stored as `Blob` values. Streams, devices, credentials, live provider state, and object URLs are never persisted. Revision-checked project and artifact writes share one transaction, originals are write-once, and only processed variants may be replaced. Reopening constructs fresh view-scoped object URLs, validates referenced Blobs/assets, and rebases missing resources to the nearest usable checkpoint: prompt for a missing reference, immutable original for a missing processed variant, or Live when no take remains. IndexedDB or quota failure preserves a complete memory snapshot. Retry Save reopens IndexedDB and atomically flushes that snapshot and its offline deletion journal; failed retry, original-download, and explicit tab-only recovery leave memory intact. Persistent-storage permission is requested only after an explicit media save; denial leaves truthful best-effort browser retention.
...
The v3 payload keeps prompt text, bounded metadata, nullable opaque reference-image asset IDs, the complete canonical character draft, and optional guided-design provenance. It migrates both the v2 and legacy v1 keys when v3 is absent. Existing characters receive empty Skin Tone, Body Shape, and Hair Color values without splitting legacy Appearance or Hair text, and pre-guided records receive null guided provenance. Image bytes and content URLs never enter `localStorage`. Recents are versioned by mode, canonical prompt, and exact asset ID, while saved characters keep the immutable asset version captured at save time.
...
- The Guided project repository owns allowlisted checkpoint metadata and IndexedDB media Blobs. Its view-scoped URL registry owns only hydrated object URLs and may revoke them without deleting durable media.
