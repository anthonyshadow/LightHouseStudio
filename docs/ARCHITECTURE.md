...
## Durable Project aggregate foundation
...
inserts the revision/role links, and advances the current pointer in one database transaction.
Normalized Project-to-asset/job/output tables repeat owner only to support composite ownership
foreign keys. Restrictive foreign keys preserve lineage and prevent destructive parent removal.
Archive and deleted status are lifecycle overrides; permanent delete currently writes a confirmed
tombstone and does not erase rows or bytes.

The Drizzle repository is composed in authoritative `postgres` and `neon` modes only. Existing
Saved Videos and processing jobs remain valid and unassigned; migration `0009` performs no
backfill. A job relationship pins the initiating revision so future orchestration can reject stale
result promotion without losing safe lineage. See
