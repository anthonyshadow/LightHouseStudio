...
| Project metadata, immutable snapshots, and asset/job/output relationships              | Owner-scoped Drizzle rows in authoritative `postgres`/`neon`. Snapshots retain strict creative intent and opaque durable IDs, never media bytes, object URLs, credentials, private storage locations, provider bodies, or React state. Existing records are not backfilled, and no current Project route/UI creates rows.                                                                                                                                                                  | None by itself; a later explicit provider action follows the linked job's existing transfer boundary.                   |
...
- Project archive preserves every revision and relationship. The separately confirmed permanent
  ...
  or media bytes. Restrictive relationships prevent physical parent deletion while retained
  lineage remains. No automatic Project garbage collection, account erasure, or backup-expiry
