...
| Provider preview audio                                                                                                 | Fetched through a 2 MiB broker/browser ceiling into one short-lived browser Blob URL; aborted/revoked on replacement or unmount and never persisted                               | ElevenLabs/provider preview storage                                                                                                                                    |
...
ElevenLabs conversion output remains temporary browser media. The pinned five-minute
`mp3_44100_128` result has an inclusive 8 MiB ceiling at both the broker and browser. Overflow,
malformed output, and cancellation abort the in-flight provider reader where supported and do not
replace, delete, or mutate the immutable original or last valid take.
