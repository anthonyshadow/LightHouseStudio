...
| Campaign and Project metadata, immutable snapshots, and relationships                  | Owner-scoped v2 files in `local`/`shadow` or Drizzle rows in authoritative `postgres`/`neon`. Campaign retains name/optional brief/lifecycle only. Project membership is nullable and same-owner. Snapshots and media/job/output relationships remain Project-owned and never store bytes, object URLs, credentials, private storage locations, provider bodies, or React state. Existing Projects migrate with null membership; other records are not backfilled.                         | None by itself; a later explicit provider action follows the linked job's existing transfer boundary.                   |
