...
| Recipes, characters, outfits, wardrobe, and voice preferences                          | Sanitized/versioned user-namespaced `localStorage`; in `neon`, revision-CAS sync copies canonical metadata to PostgreSQL. Browser divergence pauses sync without overwrite.                                                                         | None, except later explicit provider work.                                                                              |
...
| Saved videos, immutable versions, thumbnails, and attribution                          | Owner-scoped files/manifests in `local`; Drizzle rows plus registered local or private R2 bytes in `neon`; `shadow` retains local authority and may copy new video bytes to R2. Logical deletion retains detached bytes pending approved GC policy. | None unless the operator later uses a loaded version in an explicit provider action.                                    |
| Uploaded/generated/edited/composed references                                          | Immutable owner-scoped local files or private R2 objects with local/Drizzle metadata.                                                                                                                                                               | Upload/direct save: none; image-provider actions transfer required prompt/options/source bytes.                         |
...
- remove a dedicated `LIGHTFRAME_DATA_DIR` only after resolving and reviewing the exact target; and
