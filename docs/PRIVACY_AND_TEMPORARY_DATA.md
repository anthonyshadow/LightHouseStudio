...
| Campaign and Project metadata, immutable snapshots, source records, and relationships  | Owner-scoped v3 files in `local`/`shadow` or Drizzle rows in authoritative `postgres`/`neon`. Campaign retains name/optional brief/lifecycle only. Project membership is nullable and same-owner. One accepted Project source retains normalized inspection metadata, checksum privately, accepting revision, ready asset, and optional exact Saved Video Version lineage. Snapshots/relations never store bytes, object URLs, credentials, private storage locations, provider bodies, or React state. | None from source selection, upload, recording acceptance, hydration, or resume; a later explicit provider action follows its own transfer boundary. |
| Pending Project session proposal                                                       | Bounded current-tab memory under the Project ID from `/studio/projects/:projectId`; contains only workflow phase, explicit live-session metadata, and server Project/revision base tokens. It is cleared after save/discard/exit/logout and is lost on crash, confirmed reload, tab close, or browser eviction. It never enters IndexedDB, `localStorage`, URLs, logs, or media ownership.                                                                                                              | Loopback Project checkpoint API only; no provider receives it.                                                                                      |
