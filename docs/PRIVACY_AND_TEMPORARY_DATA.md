...
| Submit selected Character Swap     | Startup-selected Decart or Pruna receives a synthetic-named compatible video, normalized prompt/options, and validated reference when required. Pruna receives one reference, configured resolution, and an ephemeral locally prepared MP4 for MOV/WebM input; selecting no visual edit stays local |
...
for ElevenLabs, visual batch contact, and remote reference import, validates/bounds inputs and outputs, owner-scopes
