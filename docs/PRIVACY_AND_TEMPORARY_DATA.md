...
| Project metadata, immutable snapshots, and asset/used-Version/job/output relationships | Owner-scoped Drizzle rows in authoritative `postgres`/`neon`. Snapshots are strictly parsed/canonicalized and retain only creative intent and opaque durable IDs. Revision-scoped normalized relations prove exact same-owner media; job/output rows retain immutable initiating/producing revisions. They never store bytes, object URLs, credentials, private storage locations, provider bodies, or React state. Existing unrelated records are not backfilled, and no current Project route/UI creates rows. | None by itself; a later explicit provider action follows the linked job's existing transfer boundary.                   |
